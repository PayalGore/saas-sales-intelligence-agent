...
deals_df: deal_id, account_id, account_name, deal_type, deal_value, stage, probability, create_date, expected_close_date, actual_close_date, sales_rep, region, segment, product, loss_reason
  ...
  - loss_reason is ONLY populated for stage='Closed Lost'. For other stages it is NULL.
  - loss_reason values: 'Price too high', 'Went with competitor', 'Budget cut', 'No decision', 'Timeline mismatch', 'Product gap'
...
IMPORTANT: When asked about underperformers, query rep_performance and sort by win_rate ASC.
When asked about loss reasons by rep, query rep_loss_reasons.
When asked about loss reasons overall, query loss_reasons.
...
Return ONLY SQL. No markdown."""
        ...
        for attempt in range(3):
            resp = client.chat.completions.create(model="gpt-4o-mini",
                messages=[{"role": "system", "content": "SQL expert. SQLite only. No date functions. Use the exact table and column names provided."},
            ...
            try:
                ...
                return {"success": True, "data": result.to_string(), "row_count": len(result), "sql_used": sql,
                        "columns": list(result.columns), "raw_values": result.to_dict(orient="records")}
    ...
    def generate_weekly_report():
        ...
        return {
            ...
            "loss_summary": loss_reasons.to_string(),
...
# ─────────────────────────────────────────────
# HALLUCINATION GUARD v2 — FIXED accuracy issues
# ─────────────────────────────────────────────
...
def validate_response(text, tool_results):
    """
    v2 improvements:
    - Don't skip small percentages (the old code skipped % < 30, missing win rates like 19.5%)
    - Better list numbering detection
    - Extract source numbers more thoroughly
    """
    # Extract all numbers from LLM response
    numbers = re.findall(r'\$?([\d,]+\.?\d*)\s*(%|M|K)?', text)

    # Build comprehensive source number set from tool results
    source_nums = set()
    ...
    # Also extract from raw_values if present (new in v2)
    for key, val in tool_results.items():
        if isinstance(val, dict):
            if "raw_values" in val:
    ...
    # Detect list numbering patterns: "1. ", "2. ", etc.
    list_numbers = set()
    for m in re.finditer(r'(?:^|\n)\s*(\d+)\.\s', text):
        list_numbers.add(float(m.group(1)))

    for num_str, unit in numbers:
        try:
            num = float(num_str.replace(',',''))

            # Skip list numbering (1., 2., 3., etc.)
            if num in list_numbers and num < 20:
                continue

            # Skip very small non-percentage numbers (likely formatting)
            if unit != "%" and num < 5:
                continue

            # v2 FIX: Do NOT skip small percentages — win rates like 19.5% are real data
            # Old code had: if unit == "%" and num < 30: continue  ← THIS CAUSED 0% accuracy

            # Skip known RAG benchmarks ONLY if they appear as context references
            if num in RAG_BENCHMARKS:
                # Check if this number appears in source data too
                if not any(abs(num-s)/max(s,0.01) < 0.01 for s in source_nums if s > 0):
                    continue
...
# ─────────────────────────────────────────────
# INLINE CHART GENERATION (NEW in v2)
# ─────────────────────────────────────────────
def generate_inline_chart(question, tool_results, deals_df, monthly_metrics, rep_performance, loss_reasons):
    """Generate a relevant chart based on the question and tool results"""
    q = question.lower()

    # Underperformer / rep questions → horizontal bar of win rates
    if any(w in q for w in ["underperform", "rep", "win rate", "lowest", "sales team", "performance"]):
        rp = rep_performance.sort_values("win_rate")
        colors = ["#ef4444" if wr < 35 else "#f59e0b" if wr < 45 else "#22c55e" for wr in rp["win_rate"]]
        fig = go.Figure(go.Bar(x=rp["win_rate"], y=rp["sales_rep"], orientation="h",
                               marker_color=colors, text=[f"{wr}%" for wr in rp["win_rate"]], textposition="outside"))
        fig.add_vline(x=35, line_dash="dash", line_color="red", annotation_text="Threshold (35%)")
        fig.update_layout(title="Rep Win Rate — Underperformers Highlighted", template="plotly_white",
                          height=320, xaxis_title="Win Rate (%)", yaxis_title="", margin=dict(l=10,r=10,t=40,b=10))
        return fig

    # Loss reasons → horizontal bar (cleaner than pie)
    if any(w in q for w in ["loss", "losing", "lost", "reason"]):
        lr = loss_reasons.sort_values("count", ascending=True)
        fig = go.Figure(go.Bar(x=lr["count"], y=lr["reason"], orientation="h",
                               marker_color="#667eea", text=lr["count"], textposition="outside"))
        fig.update_layout(title="Deal Loss Reasons", template="plotly_white",
                          height=300, xaxis_title="Number of Deals Lost", yaxis_title="", margin=dict(l=10,r=10,t=40,b=10))
        return fig

    # Trend questions → line chart
    if any(w in q for w in ["trend", "over time", "monthly", "bookings"]):
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=monthly_metrics["close_month"], y=monthly_metrics["win_rate"],
                                  mode="lines+markers", name="Win Rate", line=dict(color="#667eea", width=2.5)))
        fig.add_hline(y=40, line_dash="dash", line_color="red", annotation_text="40% benchmark")
        fig.update_layout(title="Win Rate Trend", template="plotly_white", height=300, margin=dict(l=10,r=10,t=40,b=10))
        return fig

    # At-risk → no chart needed, return None
    return None


# ─────────────────────────────────────────────
# AGENT (upgraded system prompt)
# ─────────────────────────────────────────────
TOOL_DEFS = [
    {"type": "function", "function": {"name": "query_data",
        "description": "Query sales data via natural language SQL. Use for data lookups, rep stats, loss reasons, underperformers. Has access to: deals_df, accounts_df, monthly_metrics, rep_performance, loss_reasons, rep_loss_reasons tables.",
...
- Be concise and executive-friendly. Under 250 words.
- When asked about underperformers, use rep_performance table sorted by win_rate ASC. Threshold is 35%.
- When asked about loss reasons, use loss_reasons table. These are from Closed Lost deals ONLY.
- Do NOT claim loss reasons are missing. The data has loss_reason for every Closed Lost deal.
- When combining underperformers + loss reasons, make TWO tool calls: one for rep_performance, one for loss_reasons or rep_loss_reasons.
- End every response with 1-2 specific, actionable recommendations."""},
...
# ─────────────────────────────────────────────
# VISUALIZATION HELPERS (enhanced)
# ─────────────────────────────────────────────
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown('<p class="sidebar-subtitle">AI-Powered Analytics Agent v2.0</p>', unsafe_allow_html=True)
        st.markdown("---")
        ...
        if st.session_state.query_count > 0:
            ...
            est_manual = st.session_state.query_count * 18  # ~18 min per manual analysis
            st.metric("Est. Manual Time", f"{est_manual} min")
        ...
        st.markdown("""
        **Function Calling** → AI picks tool
        **Text-to-SQL** → NL → SQL → data
        **RAG** → Business context (20 rules)
        **Hallucination Guard** → Number validation
        """)
    ...
    """, unsafe_allow_html=True)
    st.markdown('<p class="sub-header">Ask questions about your sales data in plain English. AI analyzes, validates, and responds in seconds.</p>', unsafe_allow_html=True)
    ...
    with col4:
        at_risk_count = len(accounts_df[(accounts_df["health_score"]<50)])
        st.metric("At-Risk Accounts", at_risk_count, "health < 50")
    ...
    with tab5:
        ...
        elif data_tab == "Loss Reasons by Rep": st.dataframe(rep_loss_reasons, use_container_width=True)
    ...
    if question:
        ...
        with st.chat_message("assistant"):
            ...
            # ── NEW: Inline chart if relevant ──
            chart = generate_inline_chart(question, results, deals_df, monthly, rep_perf, loss_reasons)
            if chart:
            ...
            manual_est = len(tools_used) * 18  # ~18 min per manual analysis
