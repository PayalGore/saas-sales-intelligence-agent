...
def build_source_tagged_context(all_results):
    ...
    LLM receives tagged blocks so it can only use numbers it can trace.
    """
    tagged = []
...
STRICT RULES:
1. ONLY use numbers that appear in the [SOURCE: tool_name] blocks provided
2. If a number is not in the source blocks, say "data not available" — never estimate
3. Do NOT round numbers unless the source already shows them rounded
4. Do NOT derive new metrics by combining source numbers
...
BUSINESS CONTEXT (benchmarks only — cite as context, not as data):
{rag}

Under 250 words. End with 1-2 actions."""})
