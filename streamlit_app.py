...
def validate_response(text, tool_results):
    """
    Trust layer: validate numeric claims in the LLM narrative against tool outputs.
    Fixes false 0% accuracy by ignoring RAG benchmark numbers and date parts,
    and by handling K/M units.
    """
    # Extract numbers from narrative
    numbers = re.findall(r'\$?([\d,]+\.?\d*)\s*(%|M|K)?', text)

    # Extract numeric values from tool outputs
    source_nums = set()
    ...
    # Detect date parts in the narrative (YYYY-MM or YYYY-MM-DD) and ignore them
    date_parts = set()
    for y, mo, d in re.findall(r'\b(19\d{2}|20\d{2})-(0?[1-9]|1[0-2])(?:-(0?[1-9]|[12]\d|3[01]))?\b', text):
        try:
            date_parts.add(float(y))
            date_parts.add(float(mo))
            date_parts.add(float(d))
        except:
            pass

    benchmark_pcts = {25, 30, 35, 40, 45, 50, 60, 70, 75, 80, 90, 100}
    ...
    for num_str, unit in numbers:
        try:
            ...
            # Ignore list numbering / tiny counts
            if num < 10:
                continue

            # Ignore date artifacts like 2025-06
            if num in date_parts:
                continue

            # Ignore RAG benchmark values
            if num in RAG_BENCHMARKS:
                continue

            # Ignore common benchmark percentages used in RAG context
            if unit == "%" and num in benchmark_pcts:
                continue

            # Normalize shorthand units so we compare apples-to-apples
            if unit == "K":
                num *= 1_000
            elif unit == "M":
                num *= 1_000_000
...
def main():
    ...
    # Typed question input (in addition to buttons + chat)
    st.markdown("**✍️ Or type your own question:**")
    tcol1, tcol2 = st.columns([5, 1])
    with tcol1:
        typed_question = st.text_input(
            "Custom question",
            placeholder="Example: Who is underperforming on the sales team and what are the top reasons we're losing deals?",
            key="typed_question"
        )
    with tcol2:
        ask_clicked = st.button("Ask", use_container_width=True, key="ask_clicked")

    if ask_clicked and typed_question and typed_question.strip():
