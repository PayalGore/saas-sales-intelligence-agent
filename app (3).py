"""
🤖 SaaS Sales Intelligence Agent v3.1
AI-Powered Self-Serve Analytics for B2B Sales Operations

Built by Payal Gore | GitHub: @PayalGore
v3.1 — Executive-ready responses, question-specific charts, vague query handling,
       forecast visualization, scenario simulation, production layout
"""
...
def render_ai_chart(question, tool_results, monthly_metrics, rep_performance, loss_reasons, rep_loss_reasons, client):
    """v3.1: Question-specific charts from tool results. KPI-tab charts are NOT repeated."""
    q = question.lower()

    # 1. UNDERPERFORMER + LOSS → per-rep loss breakdown (unique chart)
    if any(w in q for w in ["underperform", "losing", "loss reason"]) and any(w in q for w in ["rep", "team", "who"]):
            ...
            if isinstance(val, dict) and "raw_values" in val:
                for row in val["raw_values"]:
                    if "win_rate" in row and "sales_rep" in row:
                        try:
                            if float(row["win_rate"]) < 35: under_reps.append(row["sales_rep"])
                        except: pass
        if not under_reps:
        ...
        if under_reps:
            ...
            if not filtered.empty:
                fig = px.bar(filtered, x="count", y="loss_reason", color="sales_rep", orientation="h", barmode="group",
                             title=f"Why {', '.join(under_reps)} Are Losing Deals",
                             color_discrete_sequence=["#ef4444", "#f59e0b", "#8b5cf6"])
                fig.update_layout(template="plotly_white", height=340, yaxis_title="", xaxis_title="Deals Lost",
                                  margin=dict(l=10,r=10,t=50,b=10), legend=dict(orientation="h", yanchor="bottom", y=1.02))
                return fig, {"type": "rep_loss_breakdown"}

    # 2. FORECAST → solid historical + dashed forecast + confidence band
    for val in tool_results.values():
        if isinstance(val, dict) and "forecast_values" in val and val.get("forecast_values"):
            m = val.get("metric", "gross_bookings")
            hist, fc = pd.DataFrame(val["historical_values"]), pd.DataFrame(val["forecast_values"])
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=hist["close_month"], y=hist[m], mode="lines+markers", name="Historical",
            ...
            fig.add_trace(go.Scatter(x=cx, y=cy, mode="lines+markers", name="Forecast",
                                      line=dict(color="#f59e0b", width=2.5, dash="dash"), marker=dict(size=8, symbol="diamond")))
            fv = [float(v) for v in fc[m]]
            fig.add_trace(go.Scatter(x=list(fc["close_month"])+list(fc["close_month"])[::-1],
                y=[v*1.15 for v in fv]+[v*0.85 for v in fv][::-1], fill="toself",
                fillcolor="rgba(245,158,11,0.1)", line=dict(color="rgba(0,0,0,0)"), name="±15% Range"))
            fig.update_layout(title=f"{m.replace('_',' ').title()} Forecast", template="plotly_white", height=360,
                              margin=dict(l=10,r=10,t=50,b=10))
            return fig, {"type": "forecast"}

    # 3. SCENARIO → before/after bar
    for val in tool_results.values():
        if isinstance(val, dict) and "scenario_type" in val:
            s = val
            if s["scenario_type"] == "win_rate_lift":
                fig = go.Figure(go.Bar(x=["Current", "Projected"],
                    y=[s.get("current_win_rate",0), s.get("projected_win_rate",0)],
                    marker_color=["#94a3b8","#22c55e"],
                    text=[f"{s.get('current_win_rate',0)}%", f"{s.get('projected_win_rate',0)}%"], textposition="outside"))
                inc = s.get("projected_incremental_bookings", 0)
                fig.update_layout(title=f"+{s.get('magnitude',5)}pp Win Rate → +${inc:,.0f} Incremental Bookings",
                                  template="plotly_white", height=320, margin=dict(l=10,r=10,t=60,b=10))
                return fig, {"type": "scenario"}
            elif s["scenario_type"] == "churn_reduction":
                fig = go.Figure(go.Bar(x=["ARR at Risk","Projected Saved"],
                    y=[s.get("current_arr_at_risk",0), s.get("projected_arr_saved",0)],
                    marker_color=["#ef4444","#22c55e"],
                    text=[f"${s.get('current_arr_at_risk',0):,.0f}", f"${s.get('projected_arr_saved',0):,.0f}"], textposition="outside"))
                fig.update_layout(title="Churn Reduction Impact", template="plotly_white", height=320,
                                  margin=dict(l=10,r=10,t=50,b=10))
                return fig, {"type": "scenario"}

    # 4. SEGMENT BREAKDOWN → color-coded bar with threshold
    for val in tool_results.values():
        if isinstance(val, dict) and "dimension" in val and "raw_values" in val:
            ...
            if mc in df.columns and dim in df.columns:
                ...
                colors = ["#ef4444" if v < 35 else "#f59e0b" if v < 45 else "#22c55e" for v in df[mc]] if mc == "win_rate" else ["#667eea"]*len(df)
                fig = go.Figure(go.Bar(x=df[mc], y=df[dim], orientation="h", marker_color=colors,
                    text=[f"{v:.1f}%" if "rate" in mc else f"{v:,.0f}" for v in df[mc]], textposition="outside"))
                fig.update_layout(title=f"{mc.replace('_',' ').title()} by {dim.replace('_',' ').title()}",
                    template="plotly_white", height=max(260, len(df)*50), margin=dict(l=10,r=10,t=50,b=10))
                if mc == "win_rate": fig.add_vline(x=35, line_dash="dash", line_color="red", annotation_text="35%")
                return fig, {"type": "segment"}

    # 5. TREND with anomaly markers
    for val in tool_results.values():
        if isinstance(val, dict) and "all_values" in val and "anomalies" in val:
            m = val.get("metric", "metric")
            months, values = list(val["all_values"].keys()), list(val["all_values"].values())
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=months, y=values, mode="lines+markers",
                line=dict(color="#667eea", width=2.5), marker=dict(size=6), name=m.replace("_"," ").title()))
            for a in val.get("anomalies", []):
                ...
                fig.add_trace(go.Scatter(x=[a["month"]], y=[a["value"]], mode="markers",
                    marker=dict(size=14, color=c, symbol="x"), name=f"z={a['z_score']}"))
            fig.update_layout(title=f"{m.replace('_',' ').title()} Trend", template="plotly_white", height=320,
                              margin=dict(l=10,r=10,t=50,b=10))
            return fig, {"type": "trend"}

    # 6. AUTO from SQL query results
    for val in tool_results.values():
        if isinstance(val, dict) and "raw_values" in val and val.get("raw_values") and 2 <= len(val["raw_values"]) <= 20:
            df = pd.DataFrame(val["raw_values"])
            cat = [c for c in df.columns if df[c].dtype == "object" and df[c].nunique() <= 15]
            num = [c for c in df.columns if pd.api.types.is_numeric_dtype(df[c]) and "id" not in c.lower()]
            if cat and num:
                pri = ["win_rate","count","deal_value","total_bookings","arr","quota_attainment"]
                yc = next((c for c in pri if c in num), num[0])
                xc = cat[0]
                fig = px.bar(df.sort_values(yc, ascending=True), x=yc, y=xc, orientation="h",
                             color_discrete_sequence=["#667eea"],
                             title=f"{yc.replace('_',' ').title()} by {xc.replace('_',' ').title()}")
                fig.update_layout(template="plotly_white", height=max(260, len(df)*40), margin=dict(l=10,r=10,t=50,b=10))
                return fig, {"type": "auto"}

    # 7. FALLBACK: AI chart planner
    ...
    if spec and spec.get("dataset") and spec["dataset"] in datasets:
        df = datasets[spec["dataset"]].copy()
        if not df.empty:
            x = spec.get("x", df.columns[0])
            y = spec.get("y", df.columns[min(1,len(df.columns)-1)])
            if x not in df.columns: x = df.columns[0]
            if y not in df.columns: y = df.columns[min(1,len(df.columns)-1)]
            ct = spec.get("chart_type", "bar")
            fig = px.line(df,x=x,y=y,markers=True,title=spec.get("title","")) if ct=="line" else px.bar(df,x=x,y=y,orientation=spec.get("orientation","v"),title=spec.get("title",""))
            fig.update_layout(template="plotly_white", height=340, margin=dict(l=10,r=10,t=60,b=10))
            return fig, spec
...
def run_agent(question, client, tools_funcs, kb_embeddings):
    ...
    start = time.time()
    messages = [
        {"role": "system", "content": """You are a senior SaaS sales operations analyst presenting to VP-level leadership. You have direct access to live sales data through tools.

RESPONSE FORMAT — Always structure your answer like this:
**Summary** (1-2 sentences: the headline finding)
**Key Metrics** (the specific numbers, use exact figures from tool results)
**Analysis** (2-3 sentences: why this matters, what's driving it, comparison to benchmarks)
...
- Keep total response under 200 words. Executives skim, they don't read essays.
- For underperformers: query rep_performance sorted by win_rate ASC. Threshold is 35%.
- For loss reasons: query loss_reasons or rep_loss_reasons table. Every Closed Lost deal has a loss_reason.
- Do NOT claim data is missing. If a tool returns data, use it.
- For combined questions (underperformers + loss reasons), make MULTIPLE tool calls.
...
- When data is insufficient for the question asked, say so clearly and suggest a better question.

VAGUE QUERY HANDLING:
If the user asks something too vague like "how are things" or "tell me everything" or enters gibberish:
- Do NOT hallucinate an answer.
- Instead, respond: "I can help with specific sales analytics questions. Try asking about: win rate trends, rep performance, at-risk accounts, pipeline forecasts, or loss reasons. What would you like to explore?"

TONE: Direct, data-driven, no filler words. Write like a McKinsey analyst, not a chatbot."""},
...
# ─────────────────────────────────────────────
# MAIN APP (v3.1 — production layout)
# ─────────────────────────────────────────────
def main():
    # ── SIDEBAR ──
    with st.sidebar:
        st.markdown('<p class="sidebar-title">Sales Intelligence Agent</p>', unsafe_allow_html=True)
        st.markdown('<p class="sidebar-subtitle">v3.1 · Built by Payal Gore</p>', unsafe_allow_html=True)
        st.markdown('---')
        ...
        if st.session_state.query_count > 0:
            ...
            st.caption(f"Est. manual equivalent: **{manual_min} min** → saved {manual_min - int(st.session_state.total_time_saved/60)} min")
            st.markdown('---')

        st.markdown("**Architecture**")
        st.caption("Function Calling → Text-to-SQL → RAG (20 rules) → Forecasting → Scenario Sim → Hallucination Guard")
        st.markdown('---')
        st.markdown("[GitHub](https://github.com/PayalGore/saas-sales-intelligence-agent) · [LinkedIn](https://linkedin.com/in/payalgore)")

    # ── HEADER ──
    st.markdown("""
    <p class="main-header">
        Sales Intelligence Agent <span class="version-badge">v3.1</span>
    </p>
    """, unsafe_allow_html=True)
    st.markdown('<p class="sub-header">Natural language → Data analysis → Executive insights → In seconds, not hours.</p>', unsafe_allow_html=True)

    # ── LOAD DATA ──
    deals_df, accounts_df, monthly, rep_perf, loss_reasons, rep_loss_reasons = generate_data()

    # ── KPI BAR (compact, always visible) ──
    latest = monthly.iloc[-1]
    prev = monthly.iloc[-2]
    k1, k2, k3, k4, k5 = st.columns(5)
    with k1:
        st.metric("Bookings", f"${latest['gross_bookings']:,.0f}",
    ...
    with k4:
        ...
        st.metric("At-Risk", at_risk_n, "health < 50", delta_color="inverse")
    with k5:
        coverage = round(float(deals_df[~deals_df["stage"].isin(["Closed Won","Closed Lost"])]["deal_value"].sum()) / max(float(latest["gross_bookings"]), 1), 1)
        st.metric("Coverage", f"{coverage}x", "pipeline/target")

    # ── AGENT SECTION (primary focus) ──
    st.markdown("---")
    st.markdown("### 💬 Ask anything about your sales data")

    # Quick buttons — organized by use case, 4 columns, 12 questions
    ...
    with q1:
        ...
        if st.button("❌ Top loss reasons", use_container_width=True):
            st.session_state.selected_question = "What are the top reasons we are losing deals and which reason has the highest count?"
    with q2:
        ...
        if st.button("📉 Slipping deals", use_container_width=True):
            st.session_state.selected_question = "Which deals are slipping past their expected close date and what is the total value at risk?"
    with q3:
        if st.button("🔮 Forecast bookings", use_container_width=True):
            st.session_state.selected_question = "Forecast gross bookings for the next 3 months based on the historical trend."
        ...
        if st.button("🧪 Win rate +5pp scenario", use_container_width=True):
            st.session_state.selected_question = "What if we improve win rate by 5 percentage points next month? What is the revenue impact?"
    with q4:
    ...
    if "selected_question" in st.session_state:
        ...
        question = st.chat_input("Type a question: e.g. 'Which reps need coaching?' or 'Forecast next quarter bookings'")
    ...
    if question:
        ...
        with st.chat_message("assistant"):
            ...
            st.markdown(f"""
            <div style="margin-top: 10px; padding: 8px 12px; background: #f8f9fa; border-radius: 8px; font-size: 0.82rem; display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                <span class="accuracy-badge {acc_class}">🔒 {accuracy}% ({verified}/{total})</span>
            ...
            with st.expander("🔍 How the AI reasoned"):
                ...
                with rc2:
                    if chart_spec:
                        st.markdown("**Chart decision:**")
                        st.caption(f"Type: {chart_spec.get('type', 'auto')}")
                    if unverified:
        ...
        full_response = f"{answer}\n\n*🔒 {accuracy}% ({verified}/{total}) | ⏱️ {elapsed}s | {', '.join(tools_used)}*"
        st.session_state.messages.append({"role": "assistant", "content": full_response})

    # ── DASHBOARD (below the agent, in expandable section) ──
    st.markdown("---")
    with st.expander("📊 Dashboard & Raw Data", expanded=False):
        ...
        with tab5:
            data_tab = st.selectbox("Table", ["Pipeline Deals", "Accounts", "Monthly Metrics", "Rep Performance", "Loss by Rep"])
            tbl_map = {"Pipeline Deals": deals_df, "Accounts": accounts_df, "Monthly Metrics": monthly,
                       "Rep Performance": rep_perf, "Loss by Rep": rep_loss_reasons}
