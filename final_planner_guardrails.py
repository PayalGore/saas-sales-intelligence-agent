...
def get_tools(deals_df, accounts_df, monthly_metrics, rep_performance, loss_reasons, client):
    ...
    def query_data(question):
        ...
        # Schema summary shown to LLM — tells it exactly what data exists
        SCHEMA = f"""
Available tables and columns:
- deals_df: deal_id, account_id, account_name, deal_type, deal_value, stage, probability,
            create_date, expected_close_date, actual_close_date, sales_rep, region, segment, product, loss_reason
  Stages: Prospecting, Qualification, Proposal, Negotiation, Closed Won, Closed Lost
  Regions: Northeast, Southeast, West, Midwest

- accounts_df: account_id, account_name, segment, region, arr, health_score, nps_score,
               renewal_date, tenure_months, support_tickets_last_90d, assigned_rep, product, industry

- monthly_metrics: close_month (format YYYY-MM), gross_bookings, deals_won, deals_lost,
                   total_deals_closed, avg_deal_size, total_pipeline_value, win_rate
  Latest month: {latest_month} | Previous month: {previous_month}

- rep_performance: sales_rep, total_bookings, deals_won, avg_deal_size, deals_lost,
                   win_rate, annual_quota, quota_attainment
  USE THIS TABLE for all win rate, quota, and rep performance questions.
  win_rate is pre-computed — do NOT calculate from deals_df.

- loss_reasons: reason, count
"""

        def build_prompt(q, err=""):
            err_note = f"\nERROR from last attempt: {err}\nWrite a simpler, corrected query." if err else ""
            return f"""You are a SQL expert. Convert this question to a SQLite query.{err_note}

{SCHEMA}

RULES:
- SQLite syntax only. No CURRENT_DATE, NOW(), or date arithmetic.
- For "last month" filter: close_month = '{previous_month}'
- For "this month" or "latest": close_month = '{latest_month}'  
- For win rates: SELECT from rep_performance — never calculate from deals_df
- For vague questions (e.g. "rep stats", "show me everything"): return all columns from the most relevant table
...
Question: {q}"""
        ...
        for attempt in range(3):
            resp = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are a SQLite expert. Return only valid SQL queries. No markdown, no explanation."},
            ...
            try:
                ...
                if len(result) == 0:
                    # Query ran but returned nothing — tell LLM and retry
                    last_error = "Query returned 0 rows. Check your WHERE conditions and try broader filters."
                    continue
                return {"success": True, "data": result.to_string(index=False), "row_count": len(result), "sql_used": sql}
            except Exception as e:
                last_error = str(e)

        # Universal fallback: ask LLM which table best answers this question
        fallback_resp = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{"role": "user", "content": f"Given this question: '{question}'\nWhich single table from [deals_df, accounts_df, monthly_metrics, rep_performance, loss_reasons] best answers it? Reply with ONLY the table name."}],
            temperature=0
        )
        table_name = fallback_resp.choices[0].message.content.strip().lower()
        if table_name in tables:
            df = tables[table_name]
            if hasattr(df, 'to_string'):
                return {{"success": True, "data": df.to_string(index=False), "row_count": len(df),
                        "sql_used": f"fallback_direct_{table_name}", "note": f"SQL failed, returning full {table_name}"}}
        return {"success": False, "error": f"Could not answer: {last_error}"}
...
def validate_response(text, tool_results):
    ...
    for num_str, unit in numbers:
        try:
            ...
            # Skip very small numbers and list formatting
            if num < 2: continue
            # Skip RAG benchmark values
            if num in RAG_BENCHMARKS: continue
            # Skip ordinal/list numbers (1, 2, 3 in "1. action item")
    ...
    # If no checkable numbers found, treat as 100% (narrative-only response)
    if total == 0:
        return 100.0, 0, 0, []
    return round((verified/total)*100, 1), verified, total, unverified
...
def resolve_ambiguity(question, client):
    """
    Guardrail 1: Only block questions completely unrelated to sales.
    Fast-pass common sales questions without calling the API.
    """
    CLEAR_PATTERNS = [
        "last month", "last week", "last quarter", "this month", "this week",
        "this quarter", "win rate", "win rates", "rep", "reps", "region",
        "accounts", "at risk", "slipping", "deals", "bookings", "revenue",
        "loss reason", "loss reasons", "report", "trend", "compare",
        "lowest", "highest", "best", "worst", "how did we do",
        "top", "bottom", "pipeline", "health check", "weekly", "quota",
        "forecast", "churn", "renewal", "performance", "team", "sales"
    ]
    q_lower = question.lower()
    if any(p in q_lower for p in CLEAR_PATTERNS):
        return {"clear": True}

    resp = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": "You validate questions for a B2B SaaS sales analytics system. Return JSON only. Return {\"clear\": true} for any sales-related question. Return {\"clear\": false, \"clarification\": \"Can you ask a sales-related question?\"} ONLY if the message is completely unrelated to business data (e.g. greetings, weather, jokes). When in doubt return clear true."},
...
def build_source_tagged_context(all_results):
    """
    Guardrail 2 — tag every number with its source tool.
    Uses flat JSON (no indent) to prevent text/number concatenation issues.
    """
    tagged = []
    for tool_name, result in all_results.items():
        tagged.append(f"[SOURCE: {tool_name}]")
        # Use flat JSON to avoid formatting artifacts in LLM output
...
RULES:
1. ONLY use numbers from the [SOURCE] blocks — never estimate or invent
2. Write numbers cleanly with a space before units: "42.9%" not "42.9%down"
3. Always put spaces around numbers: "$1,993,141" not "$1,993,141down"
4. If data is unavailable say so — do not guess
5. Concise, executive-friendly. Under 200 words.
6. End with 1-2 specific named actions (include rep names, account names where relevant).
7. When showing win rates order by lowest first."""
...
def run_agent(question, client, tools_funcs, kb_embeddings):
    ...
    # ── Guardrail 1: Ambiguity check before any tool runs ──
    ...
    if not ambiguity.get("clear", True):
        ...
        return (f"Before I pull the data — {clarification}",
                100.0, 0, 0, ["ambiguity_resolver"], elapsed, {})

    # ── Guardrail 2: Constrained system prompt ──
    messages = [
    ...
    # ── Guardrail 3: Source-tagged context + RAG ──
    source_tagged = build_source_tagged_context(all_results)
    rag = retrieve_context(question, client, kb_embeddings)

    messages.append({"role": "user", "content": f"""Generate the final answer using ONLY the data below.

DATA (use these numbers exactly as shown — copy them precisely):
{source_tagged}

BENCHMARKS FOR CONTEXT (do not present as data — use only to add commentary):
{rag}

Format requirements:
- Write numbers clearly with spaces: "$1,993,141" not "$1993141"  
- Percentages with spaces: "42.9%" not "42.9%declining"
- Under 200 words. End with 1-2 specific actions."""})

    # ── Guardrail 4: Lower temperature for final generation ──
    final = client.chat.completions.create(model="gpt-4o-mini", messages=messages, temperature=0.1)
    answer = final.choices[0].message.content

    # ── Guardrail 5: Validation backstop (should rarely trigger now) ──
...
def main():
    ...
    with st.sidebar:
        ...
        **2. Function calling** → LLM routes to right tool
        **3. Text-to-SQL** → NL → SQL → deterministic data
        **4. RAG** → business rules as context
        **5. Source-tagged output** → numbers traced to source
        **6. Hallucination guard** → backstop validation
        """)
    ...
    # Must be placed BEFORE chat history display for Streamlit to keep it sticky
    typed_question = st.chat_input("Ask anything about your sales data...")

    # Button click overrides typed input
    ...
    if question:
        ...
        with st.chat_message("assistant"):
            ...
            st.markdown(f"""
            <div style="margin-top:10px;padding:8px 12px;background:#f8f9fa;border-radius:8px;font-size:0.82rem;">
