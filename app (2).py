...
v3.1 — Question-specific charts, forecast visualization, scenario simulation, smarter validation
"""
...
def render_ai_chart(question, tool_results, monthly_metrics, rep_performance, loss_reasons, rep_loss_reasons, client):
    """
    v3.1: Smart chart routing — generates QUESTION-SPECIFIC charts from actual tool results.
    Priority: tool-result charts first, AI planner fallback last.
    """
    q = question.lower()
    spec = None

    # ── 1. UNDERPERFORMER + LOSS → per-rep loss breakdown (unique, not in KPI tabs) ──
    if any(w in q for w in ["underperform", "losing deals", "loss reason"]) and any(w in q for w in ["rep", "team", "who", "underperform"]):
        under_reps = []
        ...
        if under_reps:
            ...
            if not filtered.empty:
                fig = px.bar(filtered, x="count", y="loss_reason", color="sales_rep",
                             ...
                             title=f"Loss Reasons — {', '.join(under_reps)}",
                             color_discrete_sequence=["#ef4444", "#f59e0b", "#8b5cf6"])
                fig.update_layout(template="plotly_white", height=360, yaxis_title="", xaxis_title="Deals Lost",
                                  margin=dict(l=10, r=10, t=50, b=10),
                                  legend=dict(orientation="h", yanchor="bottom", y=1.02))
                return fig, {"type": "rep_loss_breakdown", "reps": under_reps}

    # ── 2. FORECAST → historical solid + forecast dashed + confidence band ──
    for key, val in tool_results.items():
        if isinstance(val, dict) and "forecast_values" in val and val.get("forecast_values"):
            metric = val.get("metric", "gross_bookings")
            hist = pd.DataFrame(val["historical_values"])
            fc = pd.DataFrame(val["forecast_values"])
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=hist["close_month"], y=hist[metric], mode="lines+markers",
                                      name="Historical", line=dict(color="#667eea", width=2.5), marker=dict(size=6)))
            connect_x = [hist["close_month"].iloc[-1]] + list(fc["close_month"])
            connect_y = [float(hist[metric].iloc[-1])] + [float(v) for v in fc[metric]]
            fig.add_trace(go.Scatter(x=connect_x, y=connect_y, mode="lines+markers", name="Forecast",
                                      line=dict(color="#f59e0b", width=2.5, dash="dash"), marker=dict(size=8, symbol="diamond")))
            fc_vals = [float(v) for v in fc[metric]]
            fig.add_trace(go.Scatter(
                x=list(fc["close_month"]) + list(fc["close_month"])[::-1],
                y=[v * 1.15 for v in fc_vals] + [v * 0.85 for v in fc_vals][::-1],
                fill="toself", fillcolor="rgba(245,158,11,0.1)", line=dict(color="rgba(0,0,0,0)"),
                name="±15% Range", showlegend=True))
            slope_dir = "↑" if val.get("trend_slope", 0) > 0 else "↓"
            fig.update_layout(title=f"{metric.replace('_', ' ').title()} Forecast {slope_dir}",
                              template="plotly_white", height=380, margin=dict(l=10, r=10, t=50, b=10))
            return fig, {"type": "forecast", "metric": metric}

    # ── 3. SCENARIO → before/after comparison ──
    for key, val in tool_results.items():
        if isinstance(val, dict) and "scenario_type" in val:
            s = val
            if s["scenario_type"] == "win_rate_lift":
                fig = go.Figure()
                fig.add_trace(go.Bar(x=["Current", "Projected"], y=[s.get("current_win_rate", 0), s.get("projected_win_rate", 0)],
                                      name="Win Rate (%)", marker_color=["#94a3b8", "#22c55e"],
                                      text=[f"{s.get('current_win_rate', 0)}%", f"{s.get('projected_win_rate', 0)}%"], textposition="outside"))
                fig.update_layout(title=f"Scenario: +{s.get('magnitude', 5)}pp Win Rate Impact",
                                  template="plotly_white", height=340, margin=dict(l=10, r=10, t=50, b=10))
                ...
                fig.update_layout(title="Churn Reduction Impact", template="plotly_white", height=340,
                                  margin=dict(l=10, r=10, t=50, b=10))
                return fig, {"type": "scenario"}

    # ── 4. SEGMENT BREAKDOWN → from analyze_segment_performance ──
    for key, val in tool_results.items():
    ...
    for key, val in tool_results.items():
        if isinstance(val, dict) and "all_values" in val and "anomalies" in val:
            ...
            fig.add_trace(go.Scatter(x=months, y=values, mode="lines+markers",
                                      line=dict(color="#667eea", width=2.5), marker=dict(size=6), name=metric.replace("_"," ").title()))
            for a in val.get("anomalies", []):
                ...
                fig.add_trace(go.Scatter(x=[a["month"]], y=[a["value"]], mode="markers",
                                          marker=dict(size=14, color=c, symbol="x"), name=f"Anomaly z={a['z_score']}"))
            fig.update_layout(title=f"{metric.replace('_',' ').title()} Trend + Anomalies",
                              template="plotly_white", height=340, margin=dict(l=10, r=10, t=50, b=10))
            return fig, {"type": "trend_anomalies"}

    # ── 6. SQL QUERY RESULTS → auto-detect best chart from raw_values ──
    for key, val in tool_results.items():
        if isinstance(val, dict) and "raw_values" in val and val.get("raw_values") and 2 <= len(val["raw_values"]) <= 20:
            ...
            if cat_cols and num_cols:
                ...
                fig = px.bar(df_s, x=y_col, y=x_col, orientation="h", color_discrete_sequence=["#667eea"],
                             title=f"{y_col.replace('_',' ').title()} by {x_col.replace('_',' ').title()}")
                fig.update_layout(template="plotly_white", height=max(280, len(df)*40), margin=dict(l=10, r=10, t=50, b=10))
                return fig, {"type": "auto_query_result"}

    # ── 7. FALLBACK: AI chart planner (last resort) ──
    ...
    if spec and spec.get("dataset") and spec["dataset"] in datasets:
        ...
        if not df.empty:
            x, y = spec.get("x", df.columns[0]), spec.get("y", df.columns[1] if len(df.columns) > 1 else df.columns[0])
...
def main():
    with st.sidebar:
        ...
        st.markdown('<p class="sidebar-subtitle">AI-Powered Analytics Agent v3.1</p>', unsafe_allow_html=True)
        st.markdown('---')
    ...
    st.markdown("""
    <p class="main-header">
        SaaS Sales Intelligence Agent <span class="version-badge">v3.1</span>
    </p>
    """, unsafe_allow_html=True)
    st.markdown('<p class="sub-header">Ask questions in plain English → AI analyzes data, generates charts, forecasts trends, and simulates scenarios — validated in seconds.</p>', unsafe_allow_html=True)
